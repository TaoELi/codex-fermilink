    ...
    packageManager === "bun"
      ...
      : packageManager === "pnpm"
        ...
        : packageManager === "vite-plus"
          ? "vp install -g @tel-research/codex-fermilink@latest"
...
function isVitePlusOwnedCodexInstall(packagesDir) {
  ...
  try {
    const metadata = JSON.parse(
      readFileSync(
        path.join(packagesDir, "@tel-research", "codex-fermilink.json"),
        "utf8",
      ),
    );
    if (metadata.name !== "@tel-research/codex-fermilink") {
      return false;
    }

    // Vite+ records the active global installation in
    // packages/@tel-research/codex-fermilink.json.
    ...
    const installDir = installId.startsWith("#")
      ? path.join(packagesDir, `@tel-research/codex-fermilink${installId}`)
      : path.join(packagesDir, "@tel-research/codex-fermilink", installId);
