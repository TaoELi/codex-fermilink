...
Give each agent a narrow question, the assumptions, required evidence, and a return format. Run read-only analyses in parallel. Give one implementation agent write ownership unless files are explicitly partitioned. While jobs run under deterministic monitoring, plan the next analysis rather than idling agents on watch duty.
